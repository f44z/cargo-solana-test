...
use solana_test_initializer::config::SolanaTestInitializerConfig;
...
/// Use `SolanaTestInitializerConfig::default()` value if no config or args
#[test]
fn start_no_args() {
...
fn start_with_config_no_args() {
    let mut config = SolanaTestInitializerConfig::default();
